 ...
 * @param opt
 */
export function doPopulateContextToDatePicker(dialogModel, opt) {
  ...
  for (const dateTimeInputEle of dateTimeInputEles) {
    ...
    // const fp = flatpickr(dateTimeInputEle, {
    //   locale: locale === 'en' ? null : Japanese, // このインスタンスのみの言語指定
    //   //disableMobile: "true",//モバイルモードのときにスマホnativeなpickerをだしたい場合はコメントアウト
    //   enableTime: true,   // 時間の選択可否
    //   noCalendar: false,   // カレンダー非表示
    //   dateFormat: "H:i",
    //   minuteIncrement: 1,// 分を1分ごとに上下させる
    //   //defaultDate: "13:45" //既に時刻がある場合はデフォルト時間をセットする
    // });
